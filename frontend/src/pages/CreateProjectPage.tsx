...
export default function CreateProjectPage() {
  ...
  const handleNLPParse = async () => {
    ...
    try {
      ...
      if (result.parsed.suggested_name) {
        ...
        const category =
          result.parsed.project?.product_category?.replace('_', ' ')?.replace(/\b\w/g, (c) => c.toUpperCase()) || '';
  ...
  return (
    <div className="min-h-screen py-8 bg-blue-50">
      <div className="w-full px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Create New Project</h1>
          <p className="text-gray-600 mt-2">Start your Life Cycle Assessment using Smart Input (NLP) or Manual Entry</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white rounded-xl shadow-md mb-6 overflow-hidden">
            ...
            <div className="flex border-b">
              <button
                ...
                className={`px-6 py-3 -mb-px font-medium ${
                  activeMode === 'nlp' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'
                }`}
              >
                Smart Input (NLP)
              </button>
              <button
                ...
                className={`px-6 py-3 -mb-px font-medium ${
                  activeMode === 'manual' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'
                }`}
              >
                Manual Entry
              </button>
            ...
            <div className="p-6">
              {activeMode === 'nlp' ? (
                <>
                  {/* NLP Content (previous left card content) */}
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-lg font-semibold">Product Description</h3>
                        <p className="text-sm text-gray-500">Describe your product in natural language</p>
                      </div>
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        placeholder={
                          isTranscribing
                            ...
                            : 'Example: 10kg copper wire, PVC coated, used in an electric motor for 10 years. Contains 30% recycled content.\n\nOr upload a document (PDF, DOCX, TXT) with product specifications.'
                        }
                      ...
                      <button
                        ...
                        className={`absolute right-3 top-3 p-2 rounded-lg transition flex items-center justify-center ${
                          isRecording ? 'bg-red-500 text-white animate-pulse hover:bg-red-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
  ...
  <button
    ...
    className="px-4 py-4 bg-green-600 text-white rounded-md hover:bg-green-700 transition text-sm font-medium disabled:bg-green-300 flex items-center justify-center gap-2"
  ...
  >
    {isParsingNLP ? (
      ...
      <>
        <Sparkles className="w-4 h-4" /> Parse Description
      </>
    )}
  </button>
</div>
  ...
  <Nut className="w-4 h-4 text-blue-800" />
  Materials Detected
</h4>
                              ...
                              {nlpResult.parsed.parsing_method && (
                                <span
                                  className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${
                                    nlpResult.parsed.parsing_method === 'groq_llm' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'
                                  }`}
                                >
                                  {nlpResult.parsed.parsing_method === 'groq_llm' ? (
                                    <>
                                      <img src="/images/ai.png" alt="AI" className="w-4 h-4 inline" /> AI Parsed
                                    </>
                                  ) : (
                                    <span className="text-xs px-2 py-1 rounded-full flex items-center gap-1 bg-gray-100 text-gray-600">
                                ...
                                <div key={idx} className="bg-white p-3 rounded-lg shadow-sm">
                                  <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-2">
                                      ...
                                      {mat.is_coating && <span className="bg-yellow-100 text-yellow-700 text-xs px-2 py-0.5 rounded-full">Coating</span>}
                                      {mat.is_composite && <span className="bg-indigo-100 text-indigo-700 text-xs px-2 py-0.5 rounded-full">Composite</span>}
                                    </div>
                                    <div className="flex gap-2 text-xs">
                                      <span
  className={`px-2 py-1 rounded ${
    mat.quantity ? 'bg-gray-100' : 'bg-amber-100 text-amber-700'
  }`}
>
  {mat.quantity ? (
  ...
  Project Settings
                ...
                <>
                  {/* Manual Content (previous right card content) */}
                  <div className="space-y-4">
                    <div>
      ...
      <p className="text-sm text-gray-600">
        Upload your own material/product dataset to inform design decisions
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Upload Area */}
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                <input ref={fileInputRef} type="file" accept=".csv,.xlsx,.xls" onChange={handleDatasetUpload} className="hidden" />
  ...
  <p className="text-xs text-gray-400">CSV or Excel file with materials data</p>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-xs text-gray-500 mt-2">Your data enhances the AI's recommendations for your specific context</p>
              </div>
            </div>
            ...
            {uploadedDataset && (
              <div className="mt-4 bg-green-50 p-4 rounded-lg border border-green-200">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button type="button" onClick={() => setUploadedDataset(null)} className="text-red-500 hover:text-red-700 text-sm">
                    Remove
                  </button>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  ...
                  {uploadedDataset.materials.length > 4 && (
                    <div className="bg-white p-2 rounded text-xs flex items-center justify-center text-gray-500">+{uploadedDataset.materials.length - 4} more</div>
                  )}
                </div>
              </div>
            )}
  ...
  <span className="ml-3 text-purple-600 flex items-center gap-1">
    <Database className="w-4 h-4" /> Custom Dataset Loaded
  </span>
)}

              </div>
              <div className="flex gap-3">
                <button type="button" onClick={() => navigate('/projects')} className="px-6 py-3 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition font-medium">
                  Cancel
                </button>
                <button type="submit" className="px-8 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium disabled:bg-blue-300 disabled:cursor-not-allowed flex items-center gap-2" disabled={isLoading}>
                  {isLoading ? (
                    <>
                    ...
                    <>
  <Atom className="w-5 h-5" /> Create Project
</>
